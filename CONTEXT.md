...
- **tutor** — ИИ-наставник; читает **публичную** проекцию задачи из `trainer`
  (никогда `grading`) и `current_user`. `trainer` про `tutor` не знает — цикла нет.
...
### tutor (ИИ-наставник)
Stateless-помощник в задачах тренажёра поверх внешней LLM (Anthropic Claude за
адаптером `TutorClient`). Контекст собирается из **публичной** проекции задачи
(`instructions_md`, `sheet`, `editable`) + текущих ячеек учащегося + истории диалога
(её шлёт клиент). Секретный `grading` модулю **недоступен** — наставник не может
выдать эталон. Системный промпт запрещает называть итоговое число/готовую формулу и
выносить вердикт «верно/неверно». Без своих таблиц и миграций на MVP. См. ADR-0009.
...
- Префикс `/api/v1`; роутеры: `/auth`, `/me`, `/courses`, `/quizzes`, `/trainer`, `/progression`.
  ИИ-наставник живёт под `trainer`-путём: `POST /trainer/{task_id}/assistant` (модуль `tutor`).
...
- **Tutor (ИИ-наставник)** — grounded-помощник в задаче тренажёра; ведёт к решению,
  но не называет эталон (детерминированный грейдер владеет истиной).
...
0007 аутентификация · 0008 архитектура фронтенда · 0009 ИИ-наставник.
